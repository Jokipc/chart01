...
class Penyalurankur extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'penyalurankur/index.html?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'penyalurankur/index.html?q=' . urlencode($q);
        } else {
        ...
        $config['total_rows'] = $this->Penyalurankur_model->total_rows($q);
        $penyalurankur = $this->Penyalurankur_model->get_limit_data($config['per_page'], $start, $q);
