...
class Britamabisnis extends CI_Controller
{
    ...
    { 
        ...
        if ($pn <> '') {
            $config['base_url'] = base_url() . 'britamabisnis/index.html?pn=' . urlencode($pn);
            $config['first_url'] = base_url() . 'britamabisnis/index.html?pn=' . urlencode($pn);
            $config['total_rows'] = $this->Britamabisnis_model->total_rows($pn);
            $config['per_page'] = 10;
            if($this->session->userdata('id_level')==='1'):;
            $config['total_rows'] = $this->Britamabisnis_model->total_rows($pn);
            $britamabisnis = $this->Britamabisnis_model->get_limit_data1($config['per_page'], $start, $pn);
            else:   
            $config['total_rows'] = $this->Britamabisnis_model->total_rows1($pn,$pn1);    
            $britamabisnis = $this->Britamabisnis_model->get_limit_data($config['per_page'], $start, $pn, $pn1);
            endif;
        ...
        $data = array(
            ...
            $config['total_rows'] = $this->Britamabisnis_model->total_rows($pn1);
            $config['per_page'] = 10;
            $britamabisnis = $this->Britamabisnis_model->get_limit_data1($config['per_page'], $start, $pn1);
        $config['page_query_string'] = TRUE;
        $this->load->library('pagination');
        $this->pagination->initialize($config);
