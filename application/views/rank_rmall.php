...
background: linear-gradient(90deg, rgba(19,200,42,1) 0%, rgba(95,242,218,1) 35%, rgba(188,235,245,1) 100%);"><marquee loop="1000" ><?php
	...
	<h5 class="animated infinite hinge"><center>PERINGKAT ALL RM</center></h5>
		...
		<td class="td-name"><center><b>Rm  </b></center></td>
		<td class="td-name"><center><b>Nilai</b></center></td>
		<td class="td-value"><center><b>Total*Bobot %</b></center></td>
			...
			<?php
				...
				foreach( $data as $val){
					...
					$totalpersen =$asen+$bsen+$csen+$dsen+$esen+$fsen+$gsen+$hsen+$isen+$jsen;
					$totaldana =($a1+$b1+$c1+$d1+$e1+$f1+$g1+$h1+$i1+$j1)/10;
				...
				foreach($isi as $n)
				{
					...
					print '<tr>
								...
								<td class="td-name">'.$n['rm_segemen'].'</td>
								<td class="td-value">'.number_format($n['pks'],1).'%</td>
								<td class="td-value"><b>'.number_format($n['hasilx'],2).'%</b></td>
								</tr>';		
					}
					else {}
				}	
